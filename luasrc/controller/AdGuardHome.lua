...
function index()
    ...
    entry({"admin", "services", "AdGuardHome", "gfw"}, cbi("AdGuardHome/gfw"),_("gfw shunt"), 2).leaf = true
    entry({"admin", "services", "AdGuardHome", "log"}, form("AdGuardHome/log"), _("Log"), 3).leaf = true
    entry({"admin", "services", "AdGuardHome", "manual"}, cbi("AdGuardHome/manual"), _("Manual Config"), 4).leaf = true
    ...
    entry({"admin", "services", "AdGuardHome", "checkgfw"}, call("check_gfw"))
    entry({"admin", "services", "AdGuardHome", "doupdate"}, call("do_update"))
    entry({"admin", "services", "AdGuardHome", "updategfw"}, call("up_date_gfw"))
...
function check_gfw()
    luci.http.write(luci.sys.exec("[ -f '/tmp/AdGuardHome_gfw_updata_log' ] && cat /tmp/AdGuardHome_gfw_updata_log"))
end

function up_date_gfw()
    luci.sys.exec("/usr/share/AdGuardHome/blacklist_full_wget download &")
end
